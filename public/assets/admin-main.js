...
function renderLibrarySetList(){
  ...
  const archiveHtml=archived.length?`<div class="library-list"><div class="library-list-head"><span>ชุดข้อสอบ</span><span>ประเภท / ภาคเรียน</span><span>วันที่เก็บ</span><span>การจัดการ</span></div>${archived.map(s=>`<article class="library-row"><div class="library-title"><span class="library-icon">📚</span><div><h3>${escapeHtml(s.title)}</h3>${s.courseName&&s.courseName!==s.title?`<p>${escapeHtml(s.courseName)}</p>`:''}</div></div><div class="library-meta"><span class="badge-pill">${escapeHtml(s.examType||'-')}</span><span class="library-term">${escapeHtml(s.academicYear&&s.semesterLabel?`${s.academicYear} / ${s.semesterLabel}`:'ไม่ระบุภาคเรียน')}</span></div><div class="library-date"><small>เก็บเข้าคลัง</small><strong>${s.archivedAt?new Date(s.archivedAt).toLocaleDateString('th-TH',{day:'numeric',month:'short',year:'numeric'}):'-'}</strong>${s.autoArchivedAt?'<em>อัตโนมัติ</em>':''}</div><div class="library-actions"><button class="btn btn-primary btn-sm" data-library-dup="${s.key}">＋ ทำสำเนา</button><button class="btn btn-ghost btn-sm" data-restore="${s.key}">↩ นำกลับ</button></div></article>`).join('')}</div>`:'<div class="empty-note">ยังไม่มีข้อสอบในคลัง</div>';

...
function renderExamSchedules(){
  ...
  host.querySelectorAll('.exam-schedule-card').forEach((card,index)=>{
    ...
    field.innerHTML=`<label>นักเรียนที่มีสิทธิ์สอบเพิ่มเติม (รายคน)</label><div class="chip-row">${item.studentIds.map((id,studentIndex)=>{const student=knownStudents.find(value=>value.studentId===id);return `<span class="chip">${escapeHtml(student?`${student.studentId} ${student.firstName} ${student.lastName} · ${student.classRoom}`:id)}<button type="button" data-remove-schedule-student="${index}:${studentIndex}">✕</button></span>`;}).join('')}</div><div class="schedule-student-picker"><input type="text" data-schedule-student-input="${index}" list="scheduleStudentList${index}" autocomplete="off" placeholder="พิมพ์รหัสหรือชื่ออย่างน้อย 2 ตัว"><datalist id="scheduleStudentList${index}"></datalist><button type="button" class="btn secondary" data-add-schedule-student="${index}">+ เพิ่มรายคน</button></div><small>ระบบจะแสดงเฉพาะรายชื่อที่ตรงกับคำค้นหา โดยใช้วันและเวลาของรอบนี้</small>`;
    card.append(field);
  });
  ...
  host.querySelectorAll('[data-schedule-student-input]').forEach(input=>{let timer;input.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>{const term=input.value.trim().toLocaleLowerCase('th-TH'),index=input.dataset.scheduleStudentInput,list=host.querySelector(`#scheduleStudentList${index}`);if(term.length<2){list.innerHTML='';return;}const matches=knownStudents.filter(student=>`${student.studentId} ${student.firstName} ${student.lastName} ${student.classRoom}`.toLocaleLowerCase('th-TH').includes(term)).slice(0,30);list.innerHTML=matches.map(student=>`<option value="${escapeAttr(student.studentId)}">${escapeHtml(`${student.firstName} ${student.lastName} · ${student.classRoom}`)}</option>`).join('');},180);});input.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();host.querySelector(`[data-add-schedule-student="${input.dataset.scheduleStudentInput}"]`)?.click();}});});

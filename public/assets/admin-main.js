...
function renderExamSchedules(){
  ...
  host.querySelectorAll('[data-sync-time-source]').forEach(select=>select.addEventListener('change',()=>{
    const index=Number(select.dataset.syncTimeSource);
    const targetName=select.value; if(!targetName) return;
    const target=s.examSchedules.find(other=>other.name===targetName); if(!target) return;
    s.examSchedules[index].availableFrom=target.availableFrom;
    s.examSchedules[index].availableUntil=target.availableUntil;
    renderExamSchedules();
  }));

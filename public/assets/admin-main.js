...
function renderExamSchedules(){
  ...
  host.querySelectorAll('.exam-schedule-card').forEach((card,index)=>{const item=s.examSchedules[index];const body=card.querySelector('.schedule-card-body');const field=document.createElement('div');field.className='field schedule-late-code-field';field.innerHTML=`<label>รหัสเข้าสอบหลังเกินเวลาทำข้อสอบหรือเข้าสอบย้อนหลัง (ถ้ามี)</label><input type="text" data-schedule-late-code="${index}" value="${escapeAttr(item.lateAccessCode||'')}" placeholder="เช่น LATE2569">`;body.append(field);});

...
function makePulseRow(item){const row=document.createElement('article');row.className='pulse-row';row.dataset.draftKey=item.draftKey;row.innerHTML='<div class="pulse-person"><b></b><p></p></div><div class="pulse-meta"><span class="pulse-status"></span><small></small></div><button class="btn btn-ghost btn-sm pulse-rescue" type="button">ปลดล็อกเครื่องใหม่</button>';row.querySelector('.pulse-rescue').addEventListener('click',()=>rescueExamDraft(item.draftKey,item.studentName||item.studentId));return row;}

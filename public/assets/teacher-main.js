...
async function apiPreviewQuestionFile(file){
  const response=await fetch('/api/teacher/question-file/preview',{method:'POST',headers:{'x-teacher-token':teacherToken||'','Content-Type':file.type||'application/octet-stream','x-file-name':encodeURIComponent(file.name)},body:file});
...
async function apiPermanentlyDeleteSet(key){ return apiFetch('/api/teacher/sets/'+encodeURIComponent(key)+'/permanent', { method:'DELETE', auth:true }); }
...
function scheduleSummaryLine(item){
  ...
  if(item.availableFrom||item.availableUntil){
    ...
    parts.push(`🕒 ${tr('เปิดสอบตลอดเวลา')}`);
  }
  if((item.studentIds||[]).length) parts.push(`👥 +${item.studentIds.length} ${tr('คน')}`);
  return parts.join(' · ');
}
function scheduleTitleLine(item){
  const tr = window.I18N ? window.I18N.t : (s=>s);
  if(!item.classes || !item.classes.length) return escapeHtml(tr(item.name));
  const periodLabel = scheduleNameForPeriod(classPeriods[item.classes[0]]);
  const roomsLabel = item.classes.join(', ');
  return periodLabel ? `${escapeHtml(roomsLabel)} <span class="schedule-period-tag">(${escapeHtml(tr(periodLabel))})</span>` : escapeHtml(roomsLabel);
}
...
function renderExamSchedules(){
  ...
  host.innerHTML=`<div class="mini-card" style="margin:14px 0;"><h3 style="margin:0 0 4px;">🗓️ ${tr('รอบสอบแยกตามห้อง')}</h3><p class="panel-sub">${tr('ระบบจัดกลุ่มตามรอบเรียนของห้องอัตโนมัติ กำหนดวันและเวลาเฉพาะเมื่อจำเป็น — เว้นว่างไว้เพื่อเปิดสอบตลอดเวลา')}</p>${s.examSchedules.map((item,index)=>{const period=classPeriods[item.classes[0]],kind=period==='บ่าย'?'afternoon':period==='ทวิภาคี'?'cooperative':'';const expanded=scheduleExpandedNames.has(item.name);return `<div class="exam-schedule-card ${kind} ${expanded?'expanded':'collapsed'}"><div class="schedule-card-head"><div><h4>${scheduleTitleLine(item)}</h4>${item.manualSplit?`<p>${tr('แยกเวลาสอบเฉพาะห้องนี้')}</p>`:''}</div><button type="button" class="btn btn-ghost btn-sm schedule-toggle-btn" data-toggle-schedule="${escapeAttr(item.name)}">${expanded?'▲ '+tr('ย่อ'):'▼ '+tr('แก้ไข')}</button></div><p class="schedule-summary-line">${scheduleSummaryLine(item)}</p><div class="schedule-card-body"${expanded?'':' hidden'}><div class="field schedule-room-field"><label>${tr('ห้องในรอบนี้')}</label><div class="chip-row">${item.classes.map((room,roomIndex)=>item.manualSplit?`<span class="chip room-chip split"><button type="button" class="chip-room-label" data-merge-room="${index}" title="${tr('คลิกเพื่อรวมห้องนี้กลับเข้ารอบเรียนเดิม')}">↩ ${escapeHtml(room)}</button><button type="button" data-remove-schedule-room="${index}:${roomIndex}">✕</button></span>`:(item.classes.length>1?`<span class="chip room-chip"><button type="button" class="chip-room-label" data-split-room="${index}:${roomIndex}" title="${tr('คลิกเพื่อแยกห้องนี้เป็นรอบเวลาของตัวเอง')}">${escapeHtml(room)} ⏱</button><button type="button" data-remove-schedule-room="${index}:${roomIndex}">✕</button></span>`:`<span class="chip">${escapeHtml(room)}<button type="button" data-remove-schedule-room="${index}:${roomIndex}">✕</button></span>`)).join('')}</div></div><div class="schedule-time-groups"><div class="schedule-time-group"><span class="schedule-time-group-label">🟢 ${tr('เริ่มสอบ')}</span><div class="schedule-time-pair"><div class="field"><label>${tr('วันที่')}</label><input type="text" data-schedule-start-date="${index}" value="${formatExamDate(item.availableFrom)}" placeholder="dd/mm/yyyy"></div><div class="field"><label>${tr('เวลา')}</label><input type="text" data-schedule-start-time="${index}" value="${formatExamTime(item.availableFrom)}" placeholder="09.00"></div></div></div><div class="schedule-time-group"><span class="schedule-time-group-label">🔴 ${tr('สิ้นสุด')}</span><div class="schedule-time-pair"><div class="field"><label>${tr('วันที่')}</label><input type="text" data-schedule-end-date="${index}" value="${formatExamDate(item.availableUntil)}" placeholder="dd/mm/yyyy"></div><div class="field"><label>${tr('เวลา')}</label><input type="text" data-schedule-end-time="${index}" value="${formatExamTime(item.availableUntil)}" placeholder="10.00"></div></div></div></div></div></div>`;}).join('')}</div>`;
  ...
  host.querySelectorAll('[data-schedule-student-input]').forEach(input=>{let timer;input.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>{const term=input.value.trim().toLocaleLowerCase('th-TH'),index=input.dataset.scheduleStudentInput,list=host.querySelector(`[data-schedule-student-results="${index}"]`);if(term.length<5){list.innerHTML='';return;}const selectedIds=s.examSchedules[Number(index)].studentIds||[];const matches=knownStudents.filter(student=>!selectedIds.includes(student.studentId)&&`${student.studentId} ${student.firstName} ${student.lastName} ${student.classRoom}`.toLocaleLowerCase('th-TH').includes(term)).slice(0,20);list.innerHTML=matches.length?matches.map(student=>`<button type="button" class="schedule-student-result" data-student-result="${escapeAttr(student.studentId)}"><b>${escapeHtml(student.studentId)}</b><span>${escapeHtml(`${student.firstName} ${student.lastName}`)}</span><em>${escapeHtml(student.classRoom)}</em></button>`).join(''):`<div class="schedule-student-empty">${tr('ไม่พบนักเรียนที่ตรงกับคำค้นหา')}</div>`;list.querySelectorAll('[data-student-result]').forEach(option=>option.addEventListener('click',()=>{input.value=option.dataset.studentResult;list.innerHTML='';input.focus();}));},180);});input.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();host.querySelector(`[data-add-schedule-student="${input.dataset.scheduleStudentInput}"]`)?.click();}});});
...
async function showQuestionAnalysis(requestedSetKey){
  ...
  try{
    ...
    document.getElementById('exportQuestionAnalysisDocxBtn').addEventListener('click',()=>downloadQuestionAnalysisDocx(setKey,true));
...
function buildRosterPrintHtml(rosters,options){
  ...
  @font-face{font-family:"THSarabunPSK";src:url("/assets/fonts/th-sarabun/THSarabunNew-webfont.woff") format("woff");font-style:normal;font-weight:400;font-display:block}@font-face{font-family:"THSarabunPSK";src:url("/assets/fonts/th-sarabun/THSarabunNew_bold-webfont.woff") format("woff");font-style:normal;font-weight:700;font-display:block}@page{size:A4 portrait;margin:0}*{box-sizing:border-box}body{font-family:"THSarabunPSK","TH Sarabun New",Tahoma,sans-serif;color:#111;margin:0;font-size:17px}.print-tools{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px;padding:10px 12px;border:1px solid #bfdbfe;border-radius:8px;background:#eff6ff;font-family:Tahoma,sans-serif;font-size:14px}.print-tools button{font:inherit;padding:7px 16px;border:0;border-radius:7px;background:#1d4ed8;color:#fff;cursor:pointer;white-space:nowrap}.document{display:flex;flex-direction:column;min-height:${documentHeight.toFixed(2)}mm;zoom:${scale.toFixed(3)}}.roster-block+.roster-block{margin-top:20px}.header{display:grid;grid-template-columns:minmax(0,1fr) 220px;gap:14px;align-items:end;margin-bottom:8px}.school{display:flex;align-items:center;gap:13px}.school-logo{width:88px;height:88px;object-fit:contain;flex:0 0 auto;filter:grayscale(1) contrast(1.25)}.school h1{font-size:14px;line-height:1.2;margin:0 0 3px}.school p{font-size:14px;line-height:1.2;margin:1px 0}.class-line{font-size:14px;line-height:1.2;font-weight:800;color:#111;margin-top:4px!important}.exam-head{text-align:center;font-size:16px;line-height:1.12}.exam-head p{font-size:16px;line-height:1.12;margin:1px 0}.exam-head h2{font-size:16px;line-height:1.12;text-decoration:underline;color:#111;margin:0 0 5px;font-weight:800}.exam-head strong{font-size:16px;line-height:1.12}.exam-head strong+p{font-size:16px;line-height:1.12;font-weight:800;margin:2px 0}.exam-head p:last-child{font-size:16px;line-height:1.12;font-weight:700;white-space:nowrap}table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:14px}th,td{border:1px solid #111;padding:3px 4px;height:26px;text-align:center;vertical-align:middle;line-height:1.08;overflow:hidden}th{font-size:14px;font-weight:800;padding-top:5px;padding-bottom:5px;line-height:1.15}th:nth-child(1){width:7%}th:nth-child(2){width:14%}th:nth-child(3){width:36%}th:nth-child(4),th:nth-child(5),th:nth-child(6),th:nth-child(7),th:nth-child(8){width:6%}th:nth-child(9){width:13%}.student-id{white-space:nowrap;font-variant-numeric:tabular-nums}.student-id-long{font-size:14px;letter-spacing:-.2px}.student-name{text-align:left;padding-left:8px;white-space:nowrap;text-overflow:ellipsis}.details{margin-top:auto;padding-top:14px;font-size:16px;line-height:1.2;font-weight:500}.details p{margin:1px 0}.label{color:#111;font-weight:800}.exam-link{color:#111;text-decoration:none;overflow-wrap:anywhere}@media print{body{padding:11mm}.print-tools{display:none}}
  .school h1,.class-line{font-size:16px;line-height:1.35}.school p:not(.class-line){font-size:12px;line-height:1.35}.exam-head,.exam-head h2,.exam-head strong,.exam-head p,.exam-head strong+p,.exam-head p:last-child{line-height:1.3}.details{line-height:1.4}.details p{margin:2px 0}table,th,.student-id-long{font-size:12px}

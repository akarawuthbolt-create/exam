...
test('auto archive uses the latest recorded exam date and waits a full 30 days', () => {
  const db = { sets: [{ key:'old', examSchedules:[{ availableFrom:'2026-01-01T10:00:00Z' },{ availableFrom:'2026-01-05T10:00:00Z' }], quickOpen:true }, { key:'recent', availableFrom:'2026-01-20T10:00:00Z' }] };
...
test('auto archive ignores unscheduled, already archived, and deleted exams', () => {
  const db = { sets: [{ key:'none' },{ key:'archived', archived:true, availableFrom:'2020-01-01' },{ key:'deleted', deletedAt:'2020-01-02', availableFrom:'2020-01-01' }] };
...
test('auto archive normalizes legacy Buddhist years before calculating 30 days', () => {
  const db = { sets: [{ key: 'buddhist-year', examSchedules: [{ availableFrom: '2569-07-21T07:30:00.000Z' }] }] };
...
test('auto archive uses regular schedules instead of stale set dates or absence-only rounds', () => {
  const set = {
    ...
    availableFrom: '2026-12-31T10:00:00.000Z',
    examSchedules: [
      { availableFrom: '2026-07-21T07:30:00.000Z' },
      { availableFrom: '2026-08-20T07:30:00.000Z', absenceOnly: true }
    ]
  };
